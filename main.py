...
• Big green  “SCANNED”  drawn on each accepted code (no pop-ups, no countdown)
"""
...
import cv2, requests, tkinter as tk
from tkinter import ttk, messagebox, filedialog
from PIL import Image, ImageTk

SCAN_COOLDOWN = 10                       # seconds before the same QR allowed again
...
# ---------- 1. GUI ----------
root = tk.Tk(); root.title("QR Run-Timer"); root.geometry("740x470")
...
def refresh_table():
    ...
    for pid, rec in runs.items():
        dur = str(rec["end"] - rec["start"]).split(".")[0] if rec["start"] and rec["end"] else ""
        view.insert("", tk.END, values=(pid, iso(rec["start"]), iso(rec["end"]), dur))
    root.title(f"QR Run-Timer – {len(runs)} runners")
...
def handle_scan_preview(camera_index: int = 0):
    ...
    def update():
        ok, frame = cap.read()
        if not ok: win.after(15, update); return

        found, infos, pts_list, _ = det.detectAndDecodeMulti(frame)
        if found:
            for data, pts in zip(infos, pts_list):
...
def export_csv():
    ...
    with open(path, "w", newline="", encoding="utf-8") as fp:
        ...
        for pid, rec in runs.items():
            sec = int((rec["end"] - rec["start"]).total_seconds()) if rec["start"] and rec["end"] else ""
            w.writerow([pid, iso(rec["start"]), iso(rec["end"]), sec])
    messagebox.showinfo("Export complete", f"Saved to:\n{path}")

# ---------- 5. BUTTONS ----------
btns = tk.Frame(root); btns.pack(fill=tk.X, padx=6, pady=(0, 6))
tk.Button(btns, text="Scan QR (preview)", height=2, width=18,
